...
def _filter(events: list[dict[str, Any]], country: str, days: int) -> list[dict[str, Any]]:
...
def economic_calendar(
    ...
    if not force_refresh and cached and _cache_is_fresh(cached):
        return _filter(cached["events"], country, days)

    try:
        events = _download()
        _write_cache(events)
        return _filter(events, country, days)
    except Exception:
        if cached:
            return _filter(cached["events"], country, days)
        raise
